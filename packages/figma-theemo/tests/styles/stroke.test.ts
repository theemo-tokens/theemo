...
describe('Styles | Stroke', () => {
  ...
  beforeEach(() => {
    ...
    adapter = new StrokeStyleAdapter(node, container);
  });
  ...
  test('correct properties', () => {
    expect(adapter.type).toBe(StyleTypes.Stroke);
  ...
  test('create reference', () => {
    adapter.linkOrigin('origin-style');
    adapter.createReference(origin.id, 'stroke-reference');

    expect(adapter.from.name).toBe('origin-style');
    expect(adapter.from.id).toBe(origin.id);

    expect(adapter.local.name).toBe('stroke-reference');
